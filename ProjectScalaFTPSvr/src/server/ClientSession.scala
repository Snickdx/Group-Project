...
class ClientSession(client : Socket, user : User) extends Runnable {
   ...
   */
  def rnto(filename : String) : String = clientCommands.readLine() match {
    case RNTO(newname) => 
      
      val oldFile = new File(user.dept + "\\" + filename)
  ...
  def processClientRequest(request : String) = request match{
    ...
    case RNTO(filename) => "503 RNFR must precede RNTO"
